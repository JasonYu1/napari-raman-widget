...
\vfill
{\footnotesize
Documentation edition based on the public source; editorial revision by Jason W. Yu.\\
\href{https://github.com/JasonYu1/napari-raman-widget}{github.com/JasonYu1/napari-raman-widget}}
\clearpage
\tableofcontents
...
Read mode & FVB & FVB preserves the standard spectrum acquisition. Single-track
uses configurable center and height detector rows (defaults 185 and 140). Image
returns the full two-dimensional detector frame.\\
Save as & blank = display only & Optional base filename. Detector data,
acquisition settings, and timing are saved together in one xarray
\code{.zarr} dataset under the active working directory.\\
\endfieldtable
...
\code{<output folder>/<name>.zarr}. Its dataset attributes include collection
start and end timestamps and the measured elapsed time in seconds, along with
the detector mode and acquisition settings. With a blank name, the data are
displayed without creating files.
...
Read mode & FVB spectrum, single-track spectrum, or 2-D detector image.\\
Save as & Optional base name for one xarray/Zarr dataset; blank disables saving.\\
...
Point spectrum & Optional \code{<name>.zarr} dataset with timing metadata; plot window.\\
