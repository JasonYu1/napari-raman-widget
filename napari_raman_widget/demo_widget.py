"""Standalone hardware-free Raman demonstration widget."""
...
class _DemoCoordinateTransformer:
    """Map normalized brightfield coordinates into simulated galvo volts."""

    def BF_to_RM(self, X, Y=None):
        coordinates = np.asarray(X, dtype=float)
        if coordinates.ndim == 1:
            if Y is None:
                if coordinates.shape != (2,):
                    raise ValueError(
                        "coordinates must contain x and y values"
                    )
...
class DemoWidget(QWidget):
    def __init__(
        ...
        *,
        ...
        # Compatibility fields used by shared workflow code. They are not
        # displayed and are never used to load physical devices.
        ...
        self.demo_mode_check.setToolTip(
            "Use simulated microscope, DAQ, and Raman hardware."
        )
        self.demo_mode_check.toggled.connect(self._update_connection_mode)
        loading_layout.addWidget(self.demo_mode_check)
        # Demo mode has its own DemoWidget and launch_demo_napari.py entry
        # point.  Keep this internal switch for that subclass, but do not show
        # it in the production hardware widget.
        self.demo_mode_check.hide()

        self.demo_controls = QWidget()
        demo_controls_layout = QVBoxLayout()

        demo_xy_row = QHBoxLayout()
        self._demo_x_label = QLabel("Stage X (um):")
        demo_xy_row.addWidget(self._demo_x_label)
        self.demo_x_input = QDoubleSpinBox()
        self.demo_x_input.setRange(-100_000.0, 100_000.0)
        self.demo_x_input.setDecimals(2)
        demo_xy_row.addWidget(self.demo_x_input)
        self._demo_y_label = QLabel("Y (um):")
        demo_xy_row.addWidget(self._demo_y_label)
        self.demo_y_input = QDoubleSpinBox()
        self.demo_y_input.setRange(-100_000.0, 100_000.0)
        self.demo_y_input.setDecimals(2)
        demo_xy_row.addWidget(self.demo_y_input)
        demo_controls_layout.addLayout(demo_xy_row)

        demo_z_row = QHBoxLayout()
        self._demo_z_label = QLabel("Stage Z (um):")
        demo_z_row.addWidget(self._demo_z_label)
        self.demo_z_input = QDoubleSpinBox()
        self.demo_z_input.setRange(-10_000.0, 10_000.0)
        self.demo_z_input.setDecimals(2)
        demo_z_row.addWidget(self.demo_z_input)
        self.demo_move_btn = QPushButton("Move stage")
        self.demo_move_btn.clicked.connect(self._move_demo_stage)
        demo_z_row.addWidget(self.demo_move_btn)
        demo_controls_layout.addLayout(demo_z_row)

        demo_image_row = QHBoxLayout()
        self.demo_snap_btn = QPushButton("Snap image")
        self.demo_snap_btn.clicked.connect(self._snap_demo_image)
        self.demo_live_check = QCheckBox("Live mode")
        self.demo_live_check.toggled.connect(self._toggle_demo_live)
        demo_image_row.addWidget(self.demo_snap_btn)
        demo_image_row.addWidget(self.demo_live_check)
        demo_controls_layout.addLayout(demo_image_row)

        self._demo_duplicate_controls = [
            self._demo_x_label,
            self.demo_x_input,
            self._demo_y_label,
            self.demo_y_input,
            ...
            self.demo_move_btn,
            self.demo_snap_btn,
            self.demo_live_check,
        ]

        demo_speed_row = QHBoxLayout()
        demo_speed_row.addWidget(QLabel("Cell motion speed:"))
        self.demo_speed_input = QDoubleSpinBox()
        self.demo_speed_input.setRange(0.1, 5.0)
        self.demo_speed_input.setValue(1.0)
        self.demo_speed_input.setSingleStep(0.1)
        self.demo_speed_input.setDecimals(1)
        self.demo_speed_input.setSuffix("x")
        self.demo_speed_input.valueChanged.connect(self._update_demo_speed)
        demo_speed_row.addWidget(self.demo_speed_input)
        demo_controls_layout.addLayout(demo_speed_row)

        self.demo_controls.setLayout(demo_controls_layout)
        self.demo_controls.setVisible(False)
        self.demo_controls.setEnabled(False)
        loading_layout.addWidget(self.demo_controls)

        self.demo_live_timer = QTimer(self)
        self.demo_live_timer.setInterval(150)
        self.demo_live_timer.timeout.connect(self._update_demo_live_frame)
        ...
        self.connect_btn = QPushButton("Restart demonstration")
        ...
        self.sel_cy_input.setValue(256)
        ...
        self.sel_cx_input.setValue(256)
        ...
        self._configure_standalone_demo()

    def _configure_standalone_demo(self) -> None:
        """Apply demo-only UI defaults and connect the simulated core."""
        self.demo_mode_check.setChecked(True)
        self.demo_mode_check.hide()
        self.connect_btn.hide()
        self.disconnect_btn.hide()
        self.reload_tf_btn.hide()
        self.hardware_config_controls.hide()
        ...
        for widget in self._px2stage_widgets:
            widget.hide()
        self.loading_box.toggled.connect(
            lambda _checked: self._apply_demo_visibility()
        )

        for combo in (self.grid_af_combo, self.sel_af_combo):
            combo.clear()
            combo.addItem("software")
            combo.setCurrentText("software")
            combo.setEnabled(False)
            combo.setToolTip("Demo mode uses software autofocus only.")

        if self.sel_cellpose_combo.findText("nuclei") >= 0:
            self.sel_cellpose_combo.setCurrentText("nuclei")
            self.sel_cellpose_combo.setToolTip(
                "The nuclei Cellpose model is the best match for simulated spheres."
            )

        if self.mda_seg_model_combo.findText("nuclei") < 0:
            self.mda_seg_model_combo.addItem("nuclei")
        self.mda_seg_track_check.toggled.connect(
            self._default_demo_tracking_model
        )
        self.demo_live_timer.setInterval(
            int(round(_DEMO_BASE_TIMING_SECONDS * 1000))
        )
        self._connect_single_core_demo()

    def _default_demo_tracking_model(self, checked: bool) -> None:
        """Use the nuclei model whenever demo tracking is enabled."""
        if checked:
            self.mda_seg_model_combo.setCurrentText("nuclei")

    def _apply_demo_visibility(self) -> None:
    ...
    def _connect_single_core_demo(self) -> None:
        """Install FakeDemoCamera on the core shared with MM dock widgets."""
        import threading
        ...
        loaded_devices = {str(device) for device in core.getLoadedDevices()}
        if not (loaded_devices - {"Core"}):
            core.loadSystemConfiguration()
        configure_demo_channels(core)
        if not hasattr(core, "_lock") and not hasattr(core, "lock"):
            core.lock = threading.RLock()

        width = int(core.getImageWidth())
        height = int(core.getImageHeight())
        world = DemoWorld(image_width=width, image_height=height)
        self._fake_camera = FakeDemoCamera(
            world,
            timing=_DEMO_BASE_TIMING_SECONDS,
            core=core,
        )
        self._connect_demo_backend(
            create_demo_backend(core=core, world=world)
        )
        ...
        self.default_engine = core.mda.engine
        self.status.setText(
    ...
    def _make_point_transformer(self, size_px, n):
        ...
        from .demo.transformer import make_demo_point_transformer

        return make_demo_point_transformer(
    ...
    def _prepare_for_selection(self):
        ...
        try:
            ...
            if not seq.stage_positions:
                stage_x, stage_y = self.core.getXYPosition()
                replacements["stage_positions"] = [
                    {
                        "x": float(stage_x),
                        "y": float(stage_y),
                        "z": float(self.core.getPosition()),
                    }
                ]
            replacements["channels"] = (
                Channel(config="BF", exposure=10.0),
            )
    ...
    # -------- loading actions --------
    def _update_connection_mode(self, demo_enabled: bool) -> None:
        """Keep the standalone widget in simulation mode."""
        self.connect_btn.setText("Restart demonstration")
        self.demo_controls.setVisible(True)

    def _set_demo_image(self, image) -> None:
        """Show a simulated camera frame in one reusable napari layer."""
        world = getattr(getattr(self, "demo_backend", None), "world", None)
        if world is not None:
            image = world.render_color_image()
        layer_name = "Demo sample"
        try:
            layer = self.viewer.layers[layer_name]
            layer.data = image
            layer.rgb = image.ndim == 3 and image.shape[-1] in (3, 4)
    ...
    def _snap_demo_image(self) -> None:
        """Acquire and display one frame from the simulated camera."""
        if self.demo_backend is None or self.core is None:
            self.status.setText("Status: demonstration is not connected")
            return
        self.core.snapImage()
        self._set_demo_image(self.core.getImage())
        self.status.setText("Status: demonstration image snapped")

    def _update_demo_live_frame(self) -> None:
        """Advance the virtual sample and display the next live frame."""
        if self.demo_backend is None or self.core is None:
            self._toggle_demo_live(False)
            return
        self.demo_backend.world.time_index += 1
        self.core.snapImage()
        self._set_demo_image(self.core.getImage())

    def _update_demo_speed(self, speed: float) -> None:
        """Set simulated cell-motion speed by changing the live frame rate."""
        speed = max(float(speed), 0.1)
        timing = _DEMO_BASE_TIMING_SECONDS / speed
        self.demo_live_timer.setInterval(max(60, int(round(timing * 1000))))
        fake_camera = getattr(self, "_fake_camera", None)
        if fake_camera is not None:
            fake_camera.timing = max(0.09, timing)

    def _toggle_demo_live(self, enabled: bool) -> None:
        """Start or stop timer-driven simulated live imaging."""
        if enabled and self.demo_backend is None:
            self.demo_live_check.blockSignals(True)
            self.demo_live_check.setChecked(False)
            self.demo_live_check.blockSignals(False)
            self.status.setText("Status: connect demonstration mode first")
            return
        if enabled:
            self.core.startContinuousSequenceAcquisition(150)
            self.demo_live_timer.start()
            self.status.setText("Status: demonstration live mode running")
        else:
            self.demo_live_timer.stop()
            if self.core is not None and self.demo_backend is not None:
                self.core.stopSequenceAcquisition()
            if self.demo_live_check.isChecked():
                self.demo_live_check.blockSignals(True)
                self.demo_live_check.setChecked(False)
                self.demo_live_check.blockSignals(False)

    def _move_demo_stage(self) -> None:
        """Move the simulated XYZ stage and refresh the camera image."""
        if self.demo_backend is None or self.core is None:
            self.status.setText("Status: demonstration is not connected")
            return
        x = float(self.demo_x_input.value())
        y = float(self.demo_y_input.value())
        z = float(self.demo_z_input.value())
        self.core.setXYPosition(x, y)
        self.core.setPosition(z)
        self._snap_demo_image()
        self.status.setText(
            f"Status: demonstration stage moved to "
            f"X={x:.2f}, Y={y:.2f}, Z={z:.2f} um"
        )

    def _connect_demo_backend(self, backend=None) -> None:
        """Connect this widget to the simulated microscope backend."""
        backend = backend or create_demo_backend()
        self.demo_backend = backend
        self.core = backend.core
        self.daq = backend.daq
        self.collector = backend.collector
        self._configure_collect_detector_rows()
        self.transformer = getattr(
            backend, "transformer", _DemoCoordinateTransformer()
        )
        self.default_engine = self.core.mda.engine
        self.vandermonde = None

        self._set_demo_image(self.core.snap())
        x, y = self.core.getXYPosition()
        self.demo_x_input.setValue(x)
        self.demo_y_input.setValue(y)
        self.demo_z_input.setValue(self.core.getPosition())
        self.demo_controls.setEnabled(True)
        self.calib_box.setEnabled(False)
        self.calib_box.setToolTip(
            "Laser aiming calibration is not used by the fake collector."
        )
        ...
        self.status.setText(
            "Status: demonstration connected OK "
            "(simulated image and Raman hardware)"
        )
    ...
    def connect(self):
        """Reconnect the simulator; this widget never opens real hardware."""
        if self.demo_backend is None:
            self._connect_single_core_demo()
    ...
    def disconnect(self):
        ...
        self.calib_box.setToolTip("")
        self._update_connection_mode(self.demo_mode_check.isChecked())
    ...
    def _set_scan_imaging_channel(self):
        """Select a valid simulated bright-field camera channel."""
        configs = list(self.core.getAvailableConfigs("Channel"))
        if configs:
            preferred = "BF" if "BF" in configs else configs[0]
            self.core.setConfig("Channel", preferred)

    def _set_scan_raman_mode(self, enabled):
        """No shutter operation is required by the simulated collector."""
        return
    ...
    def collect_dark_noise(self):
        """Stop demo live acquisition and save repeated dark spectra."""
        ...
        try:
            ...
            self.status.setText(
                f"Status: collecting {repeats} simulated dark spectra..."
            )
            self.repaint()
            dark_noise = self.collector.collect_spectra_image_points(
            ...
            window = SpectrumWindow(
                dark_noise,
                title=(
                    f"Dark noise | {read_mode} | "
                    f"{repeats}x{exposure:.0f} ms"
                ),
                spectral_calibration=self.spectral_calibration,
    ...
    def collect_raman(self):
        ...
        try:
            ...
            spec = self.collector.collect_spectra_image_points(
                np.tile(pt, (N, 1)),
                exposure,
                **read_mode_options,
            )
    ...
    def run_calibration(self):
        self.status.setText(
            "Status: laser aiming calibration is disabled for fake collection"
        )
        return
    ...
    def collect_reference(self):
        ...
        try:
            ...
            focusZ = float(self.core.getPosition())
            z_offsets = np.linspace(-search_range, search_range, search_pts)
            all_raman = []
            for z_offset in z_offsets:
                self.core.setZPosition(focusZ + float(z_offset))
                all_raman.append(
                    self.collector.collect_spectra_image_points(
                        np.tile(pt, (N, 1)), exp
                    )
                )
            all_raman = np.asarray(all_raman)
            coarse_raman = np.mean(all_raman, axis=1)
            log.append(
                f"Fake axial scan at point {point_index}: "
                f"{search_pts} z positions, {N} spectra per position.\n"
            )
    ...
    def run_grid_scan(self):
        ...
        try:
            ...
            with _StdoutRedirector(log):
                ...
                # Fake collection samples camera pixels directly; no
                # aiming calibration or galvo transform is involved.
                volts = np.full_like(grid, np.nan, dtype=float)
                self.core.stopSequenceAcquisition()
                ...
                for i, dz in enumerate(z_range):
                    ...
                    specs = self.collector.collect_spectra_image_points(
                        grid, exp
                    )
        ...
        try:
            with _StdoutRedirector(log):
                ...
                sources, autofocus_p, new_seq = automated_point_selections(
                    ...
                    direct_pixel_to_stage=True,
                    stage_settle_time=0.0,
                    image_settle_time=0.0,
                    block_mda=True,
                    show_masks=True,
                    cellpose_upsample=3,
                    cell_point_refiner=(
            ...
            self.status.setText(
                f"Status: Cellpose selected {cell_point_count} cell point(s); "
                f"quick BF MDA complete{extra} -- then Run Raman MDA"
            )
        except Exception as e:
            log.append(f"\n--- selection failed: {e} ---\n")
    ...
    def _move_clicked_to_center(self, yx):
        ...
        try:
            ...
            stage_dx, stage_dy = offset_yx[1], offset_yx[0]
            x, y = self.core.getXYPosition()
            self.core.setXYPosition(float(x + stage_dx), float(y + stage_dy))
            self.core.waitForSystem()
            self._snap_demo_image()
            self.status.setText(
                f"Status: directly centered ({yx[0]:.0f},{yx[1]:.0f}) "
                f"at ({cy},{cx}) [dX={-stage_dx:.2f}, dY={-stage_dy:.2f}]"
            )
        except Exception as e:
    ...
    def run_grid_selection(self):
        ...
        try:
            with _StdoutRedirector(log):
                ...
                origin_x, origin_y = self.core.getXYPosition()
                demo_sequence = MDASequence(
                    stage_positions=[
                        {
                            "x": float(origin_x),
                            "y": float(origin_y),
                    ...
                    channels=[{"config": "BF", "exposure": 10.0}],
                    z_plan={"range": 0.0, "step": 1.0},
                    time_plan={"interval": 0.0, "loops": 1},
                )
                point_transformer = self._make_point_transformer(sq_size, sq_n)
    ...
    def run_raman_mda(self):
        ...
        demo_cell_point_count = sum(
            len(source._points.data)
            for source in sources
            if "cell" in str(getattr(source, "name", "")).lower()
        )
        if demo_cell_point_count == 0:
            self.status.setText(
                "Status: no automated cell points -- run selection again"
            )
            return
        ...
        try:
            ...
            with _StdoutRedirector(log):
                ...
                sequence_channels = (
                    Channel(config="BF", exposure=10.0),
                    Channel(config="RM", exposure=10.0),
                )
                ...
                if "raman" in final_seq.metadata:
                    ...
                    final_seq.metadata["raman"]["channel"] = "RM"
                else:
                    print(
                ...
                print(
                    ...
                    f"demo_cell_points={demo_cell_point_count}, "
            ...
            log.append("\n--- MDA started ---\n")
            self.status.setText(
                "Status: Raman MDA started OK "
                f"({demo_cell_point_count} selected cell points, shutter=None)"
            )
        except Exception as e:
            log.append(f"\n--- MDA failed: {e} ---\n")
